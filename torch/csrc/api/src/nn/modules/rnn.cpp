...
void RNNImplBase<Derived>::check_hidden_size(
    ...
    std::string msg) {
  // yf225 TODO: this needs to be IntArrayRef compared to std::tuple
  if (hx.size() != expected_hidden_size) {
    msg = std::regex_replace(msg, std::regex("\\{1}"), c10::str(expected_hidden_size));
    msg = std::regex_replace(msg, std::regex("\\{2}"), c10::str(hx.size()));
